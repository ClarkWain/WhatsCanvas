...
- `build\Debug\WhatsCanvasUnicodeBidiConformanceTests.exe tests/unicode --exhaustive`: full Unicode 17.0.0 diagnostic run across `BidiTest.txt` and `BidiCharacterTest.txt`; this currently reports the remaining full UAX #9 gaps for BD13 isolating run sequences and deeply nested paired brackets. The latest local diagnostic run is down to 43 remaining failures out of 861,948 cases.
