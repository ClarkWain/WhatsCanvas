...
- `scripts/text_pixel_regression.bat` / `scripts/text_pixel_regression.sh`: font-only pixel regression for the `font-regression` scene against `tests/baselines/text/font-regression.ppm`; set `WHATSCANVAS_UPDATE_TEXT_BASELINES=1` to refresh the baseline.
