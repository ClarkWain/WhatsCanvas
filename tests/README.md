...
- `ctest --test-dir build-vulkan -C Debug -R ^WhatsCanvasVulkanImageFilterTests$ --output-on-failure`: real Vulkan GPU coverage for image/backdrop Gaussian blur, Clamp/Decal edges, color treatment, adaptive downsampling, render statistics, and Software pixel parity.
