...
| 性能与资源 | 流式顶点缓冲、图片命令同纹理合批、统一圆角图片的原生 shader coverage、路径命令合批、Vulkan 路径阴影 silhouette 批量提交、局部阴影栅格化 / GPU 模糊、全局 quad index buffer、离屏 render target 复用池、GPU glyph atlas 复用、indexed glyph lookup、填充三角化 / 描边网格 / 裁剪掩码 LRU 缓存、滤镜调用 / pass / 降采样 / pixel-pass 统计；统一 Release 性能套件以 1920×1080 覆盖 13 个真实帧场景，包括大量多语种文字与混合几何压力，输出 record / submit / total 分布、冷启动、吞吐、内存、像素哈希和环境指纹。 | `WhatsCanvasPerformanceSuite`、`Renderer`、`RenderTargetPool`、`GlyphAtlas`、`LruCache`、`RenderStats` |
...
- [Performance Benchmarks](doc/PERFORMANCE_BENCHMARKS.md)：记录统一三后端 1080p 帧性能套件、13 个标准场景、指标口径、JSONL 结果、版本对比、公开数据规则与可复现参考基线。
