...
[![Version](https://img.shields.io/badge/version-0.1.14-informational.svg)](CHANGELOG.md)
...
- **解析抗锯齿**：沿轮廓生成跨骑真实边缘的 ~1px 羽化带，片元着色器按覆盖度调制 alpha（细描边额外做亚像素淡化）。逐 `Paint` 开启：`paint.setAntiAlias(true)`。
