...
| Bidi run segmentation | Supported | Mixed-direction text is split into directional byte ranges before font segmentation and shaping; bidi controls are ignored as invisible formatting, leading neutral text is retained, weak-only text defaults to LTR, and RTL-base paragraphs reverse visual run order. |
...
| Bounded multiline layout | Supported | `Canvas::layoutTextBox` returns line rows, source ranges, widths, line height, and ellipsis state; line breaking supports ASCII words, tab/Unicode space separators, long unspaced tokens, and basic CJK no-space wrapping. |
