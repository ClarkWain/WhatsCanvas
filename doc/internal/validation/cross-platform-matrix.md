...
> Maintainer validation record. Public support claims must be reflected in the
> README and platform guides rather than inferred from this working matrix.
...
| Android OpenGLES host | `platforms/android/` Gradle + NDK build | `:app:assembleDebug`, `:app:assembleProfile`, `:app:assembleRelease` | `:app:lintDebug`, `:app:lintProfile`, plus API 33 AVD/manual device smoke | Debug validates diagnostics, Profile provides an installable `-O2 -DNDEBUG` performance target, and Release validates shipping configuration. Device smoke covers all packaged ABIs, UTF-8/CJK text, paths, round caps/dashes, images, portrait/landscape, and pause/resume context recreation. See [Android Integration](../../public/platforms/ANDROID_INTEGRATION.md). |
| WebAssembly + WebGL 2 host | `platforms/wasm/build.sh` with pinned Emscripten 4.0.22 | `WhatsCanvasWeb` | `platforms/wasm/test.sh` in clean headless Chrome | Reuses the canonical scene at the exact 800x400 and 400x800 logical viewports with a 3 DPR drawing buffer and 3 DPR PNG assertion. Generates the required visual-parity captures and blocks on mixed CJK/emoji rendering, resize, visibility pause/resume, WebGL context recovery, cold reload, and browser GL errors. Hardware-backed browsers must reach the display-rate gate; hosted SwiftShader/llvmpipe runs enforce forward progress and remain correctness-only evidence. |
