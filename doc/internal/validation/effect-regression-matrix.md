...
> Maintainer working matrix. The normative baseline policy is
> [`VISUAL_REGRESSION.md`](../../public/validation/VISUAL_REGRESSION.md).
...
| Shadows | `gradient-effect` validation scene | `PaintStateTests` validates shadow state and transparent-color behavior | Shadow blur is a lightweight multi-pass approximation; see `doc/public/guides/rendering/SHADOW_MODEL.md`. |
| Blend modes | showcase validation scenes using `ADD`, `SCREEN`, `SRC_IN`, and `DST_OUT` | `doc/public/guides/rendering/BLEND_MODE_AUDIT.md` records GL state mappings | Blend-heavy scenes can be compared with `scripts/compare_ppm_fuzzy.py`. |
| Strokes | showcase geometry scene and matrix/clip tests | `PaintStateTests` validates cap/join state; `StrokeTessellatorTests` preserves legacy geometry and exercises edge cases; `measureStrokeBounds` is exercised by the showcase | Stroke mesh generation uses the internal tessellator for cap/join output. |
