...
| WebAssembly + WebGL 2 host | `platforms/wasm/build.sh` with pinned Emscripten 4.0.22 | `WhatsCanvasWeb` | `platforms/wasm/test.sh` in clean headless Chrome | Reuses the canonical scene at the exact 800x400 and 400x800 logical viewports with a 3 DPR drawing buffer and 3 DPR PNG assertion. Generates all eight visual-parity captures and blocks on mixed CJK/emoji rendering, resize, visibility pause/resume, WebGL context recovery, cold reload, browser GL errors, and at-least-60-Hz requestAnimationFrame pacing. |
