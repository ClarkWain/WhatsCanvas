...
热点分析、优先实施待办和定量验收目标记录在维护者项目 backlog 中。
...
每个进程都产生了相同的场景哈希。文本改进来自已解决的 glyph-layout 缓存、零面积 glyph 缓存和四顶点索引 sprite quads。几何改进来自 move-only 命令交接、append-only 每帧路径流和确定性 AA 缓存抖动移除。几何体仍然将简单原始体扩展为通用三角形汤路径，因此其剩余差距需要维护者 backlog 中跟踪的语义原始体和索引 AA 工作。
