...
function(whatscanvas_link_fontconfig_if_available target_name)
    ...
    if (NOT TARGET PkgConfig::WHATSCANVAS_FONTCONFIG)
        pkg_check_modules(WHATSCANVAS_FONTCONFIG QUIET IMPORTED_TARGET fontconfig)
    endif()

    if (TARGET PkgConfig::WHATSCANVAS_FONTCONFIG)
        target_compile_definitions(${target_name} PRIVATE WHATSCANVAS_HAS_FONTCONFIG)
        set_property(TARGET ${target_name} PROPERTY WHATSCANVAS_FONTCONFIG_ENABLED ON)
        target_link_libraries(${target_name} PRIVATE PkgConfig::WHATSCANVAS_FONTCONFIG)
    elseif (WHATSCANVAS_FONTCONFIG_FOUND)
        set(fontconfig_libraries ${WHATSCANVAS_FONTCONFIG_LINK_LIBRARIES})
        if (NOT fontconfig_libraries)
            set(fontconfig_libraries ${WHATSCANVAS_FONTCONFIG_LIBRARIES})
        endif()
        target_compile_definitions(${target_name} PRIVATE WHATSCANVAS_HAS_FONTCONFIG)
        set_property(TARGET ${target_name} PROPERTY WHATSCANVAS_FONTCONFIG_ENABLED ON)
        target_include_directories(${target_name} PRIVATE ${WHATSCANVAS_FONTCONFIG_INCLUDE_DIRS})
        target_compile_options(${target_name} PRIVATE ${WHATSCANVAS_FONTCONFIG_CFLAGS_OTHER})
        target_link_directories(${target_name} PRIVATE ${WHATSCANVAS_FONTCONFIG_LIBRARY_DIRS})
...
function(whatscanvas_add_gl_family_library target_name project_root)
    ...
    whatscanvas_link_fontconfig_if_available(${target_name})
