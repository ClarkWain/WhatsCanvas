# Android drag performance verification
...
1024x512 card atlas containing a shared face, shared back, 13 rank masks, and 4
suit masks. The atlas is baked once from the original vector UI with the
software backend, preserving its gradients, font glyphs, card-back ornaments,
and suit paths. Cards keep only logical rank/suit/state; atlas masks are tinted
at draw time and no per-card texture is allocated.
