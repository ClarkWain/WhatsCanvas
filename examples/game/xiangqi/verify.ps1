...
& cmake -S $PSScriptRoot -B $buildDirectory "-DXIANGQI_BUILD_DESKTOP=$desktop" -DCMAKE_BUILD_TYPE=Release
...
if (-not $CoreOnly) {
    $executable = Join-Path $buildDirectory 'Release/Xiangqi.exe'
    if (-not (Test-Path -LiteralPath $executable)) { $executable = Join-Path $buildDirectory 'Xiangqi.exe' }
