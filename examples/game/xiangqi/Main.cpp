...
struct Host {
    ...
    void hover(double x, double y) {
        ...
        const bool piece = s >= 0 && game.match().position().board[s]
            && game.match().position().board[s].side == Side::Red && !game.thinking()
...
void smoke(Host& h, const std::string& capture) {
    ...
    clickSquare(h, 0, 6); h.frame(false);
    ...
    require(h.game.selected() == square(0, 6), "select pawn via host input");
    require(h.canvas.readPixelsRGBA(selected) && selected != before, "selection changes actual pixels");
    clickSquare(h, 0, 5); h.frame(false);
    ...
    h.game.undo();
    settle(h);
    clickSquare(h, 1, 7); // Capture candidate rings and quiet-move dots in the screenshot.
    h.frame(false);
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--smoke-test") smokeTest = true;
            ...
            std::cout << "Xiangqi [--difficulty easy|medium|hard] [--capture frame.ppm] [--smoke-test]\n"
    ...
    auto* window = glfwCreateWindow(1120, 820, "Xiangqi - WhatsCanvas", nullptr, nullptr);
