...
| 你关心的事项 | 当前答案 |
| --- | --- |
| **适用场景** | 原生应用和浏览器自定义 UI、工具与数据界面、HUD、2D 游戏渲染层、服务端或测试环境中的离屏图片生成。 |
...
| **成熟度** | 当前稳定 API 版本为 `1.2.1`，已定义的稳定 v1 产品范围已经完成。仓库已经建立公开 API 边界、跨平台 CI、像素回归、package consumer 集成测试、代表性移动真机验收与可审计的性能基线。 |
...
**何时需要另寻方案？**
如果项目强依赖现成的 UI 控件体系，必须使用 WebGPU，或需要严格色彩管理、文档/PDF 生成、复杂富文本编辑和长期稳定的 ABI，WhatsCanvas 暂时不合适。
...
桌面端 GitHub Release 包名为 `whatscanvas-<platform>-release-<version>.zip`，例如 `whatscanvas-win64-release-1.2.1.zip`。桌面包目录布局如下：
