...
Path::PathPoint::PathPoint(Path::Op op, const PointF &point)
    : op(op), point(point)
{
}
...
void Path::quadTo(const PointF& control, const PointF& end) {
        quadTo(control.getX(), control.getY(), end.getX(), end.getY());
    }

void Path::cubicTo(float control1X, float control1Y, float control2X, float control2Y, float endX, float endY) {
        if (!hasCurrentPoint_) {
            moveTo(endX, endY);
            return;
        }
...
void Path::cubicTo(const PointF& control1, const PointF& control2, const PointF& end) {
...
const std::vector<Path::PathPoint>& Path::getPoints() const {
        return points_;
    }
...
size_t Path::getContourCount() const {
        size_t count = 0;
...
size_t Path::getClosedContourCount() const {
        size_t count = 0;
...
RectF Path::getStrokeBounds(float strokeWidth) const {
...
std::vector<RectF> Path::getContourBounds() const {
...
bool Path::contains(const PointF& point) const {
        return contains(point.getX(), point.getY());
    }

bool Path::strokeContains(float x, float y, float strokeWidth) const {
...
bool Path::strokeContains(const PointF& point, float strokeWidth) const {
...
bool Path::pointAtLength(float targetLength, PointF& point) const {
        PointF tangent;
        return pointAndTangentAtLength(targetLength, point, tangent);
    }

bool Path::pointAndTangentAtLength(float targetLength, PointF& point, PointF& tangent) const {
...
Path Path::trim(float startLength, float endLength) const {
        Path result;
        result.setFillType(fillType_);
...
Path Path::trim(float startLength, float endLength, bool wrap) const {
        if (!wrap) {
...
Path Path::trim(float startLength, float endLength, bool wrap, bool reverse) const {
        if (!reverse) {
...
Path Path::roundedCorners(float radius) const {
...
void Path::offset(float dx, float dy) {
...
void Path::addRoundRect(const RectF& rect, float radius) {
        addRoundRect(rect, radius, radius, radius, radius);
    }

void Path::addRoundRect(const RectF& rect, float topLeftRadius, float topRightRadius, float bottomRightRadius, float bottomLeftRadius) {
        RectF normalized = normalizeRect(rect);
        const float width = normalized.getWidth();
        const float height = normalized.getHeight();
        if (width <= 0.0f || height <= 0.0f) {
...
RectF Path::normalizeRect(const RectF& rect) {
...
int Path::computeEllipseSegments(float radiusX, float radiusY) {
        constexpr float pi = 3.14159265358979323846f;
...
void Path::scaleRadiusPair(float& scale, float available, float a, float b) {
        const float sum = a + b;
...
int Path::computeCornerSegments(float radius) {
        return std::clamp(static_cast<int>(std::ceil(radius * 0.35f)), 4, 24);
    }

void Path::appendQuarterArc(float centerX, float centerY, float radius, float startAngle, float endAngle) {
        if (radius <= 0.0f) {
...
float Path::distance(const PointF& start, const PointF& end) {
        return std::hypot(end.getX() - start.getX(), end.getY() - start.getY());
    }

int Path::computeCurveSegments(float controlPolygonLength) {
